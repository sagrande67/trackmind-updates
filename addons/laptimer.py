...
# Helper centralizzato per i path della cartella scouting
# (struttura ad albero <anno>/<pista>/, vedi core/scouting_paths.py).
try:
...
class LapTimer:
    ...
    def _myrcm_salva_tempi_live(self):
        ...
        for pnum, d in self._live_pilots.items():
            ...
            # Aggancio al setup: se LapTimer e' stato lanciato dal setup,
            # marca SEMPRE il file con setup_record_id (anche per
            # transponder di altri piloti durante la sessione live).
            # Il setup_snapshot resta invece riservato al pilota del setup.
            try:
                if self.record_id:
                    sess["setup_record_id"] = self.record_id
            ...
            if _scouting_paths.salva_sessione(sess, scouting_dir, fname):
                salvati += 1
    ...
    def _schermata_timer(self):
        ...
        # v05.06.55: label sotto il cronometro che mostra cosa
        # stiamo cronometrando (categoria / fase / manche / heat).
        # Solo in modo MyRCM viene popolata col GROUP del payload;
        # in MANUALE / LIVE BLE resta vuota e occupa zero spazio.
        self._lbl_session_info = tk.Label(timer_col, text="",
                                            bg=c["sfondo"],
                                            fg=c["label"],
                                            font=self._f_info)
        self._lbl_session_info.pack()
    ...
    def _live_classifica_predittiva(pilot_info, rem_sec,
        ...
        """
        # v05.06.49: proiezione STABILE. Usiamo math.ceil per
        # quantizzare i giri aggiuntivi: cambia solo quando
        # rem_sec/passo attraversa un confine intero (= ogni
        # ~passo secondi, non ogni secondo). Cosi' la classifica
        # non "balla" e si stabilizza naturalmente man mano che
        ...
        for pid, nome, media_glob, tempi, _tot in pilot_info:
            if not tempi:
                continue
            # v05.06.52: scelta del passo gara con priorita':
            # 1. Se ho >= 3 giri locali post-baseline -> usa
            #    _live_calcola_passo (filtrato outlier, piu'
            #    preciso del MEDIUMTIME grezzo del server).
            # 2. Altrimenti se passo_reali_map ha questo pilota
            #    (= MEDIUMTIME dal server MyRCM, calcolato su
            #    TUTTI i giri inclusi pre-baseline) -> usa quello.
            # 3. Fallback: _live_calcola_passo sui pochi giri
            #    locali (anche 1).
            if len(tempi) >= 3:
                passo = LapTimer._live_calcola_passo(
                    tempi, soglia_outlier=1.4)
            elif passo_reali_map and pid in passo_reali_map \
                    and passo_reali_map[pid] > 0:
                passo = float(passo_reali_map[pid])
            else:
                passo = LapTimer._live_calcola_passo(
                    tempi, soglia_outlier=1.4)
            if passo <= 0:
                continue
            # v05.06.51: in modo MyRCM mid-session i giri pre-
            # baseline non sono in tempi[] ma il pilota ne ha gia'
            # fatti diversi (LAPS reale dal server). Il chiamante
            # puo' passare laps_reali_map e abs_reali_map (dict
            # pid -> LAPS / ABSOLUTTIME) presi direttamente dal
            # payload MyRCM, cosi' la proiezione e il rilevamento
            # ritiro usano i valori reali del pilota anche per i
            # giri pre-baseline.
            if laps_reali_map and pid in laps_reali_map:
                giri_attuali = max(int(laps_reali_map[pid]),
                                    len(tempi))
            else:
                giri_attuali = len(tempi)
            if abs_reali_map and pid in abs_reali_map:
                absoluttime = float(abs_reali_map[pid])
            else:
                absoluttime = sum(tempi)  # fallback
            ...
            if e_fermo:
                giri_aggiuntivi_int = 0
                # Tempo di "fine prevista" per pilota fermo:
                # currenttime_sec (resta dov'e')
                tempo_fine_prevista = float(currenttime_sec)
            else:
                # CEIL invece di divisione frazionaria:
                # rem_sec=180, passo=18 -> ceil = 10 (stabile)
                # rem_sec=175, passo=18 -> ceil = 10 (stabile)
                # rem_sec=170, passo=18 -> ceil = 10 (stabile)
                # rem_sec=162, passo=18 -> ceil = 9 (cambia: passa
                #   sotto i 9*18=162 secondi rimanenti)
                if rem_sec > 0:
                    giri_aggiuntivi_int = int(
                        math.ceil(rem_sec / passo))
                else:
                    giri_aggiuntivi_int = 0
                # Tempo di "fine prevista" del pilota = quando
                # finira' l'ultimo giro proiettato (assumendo
                # passo costante). Usato per ordinamento e gap
                # in tempo.
                tempo_fine_prevista = (
                    absoluttime
                    + giri_aggiuntivi_int * passo)
            giri_finali_int = giri_attuali + giri_aggiuntivi_int
            proiezioni.append({
                "pid": pid, "nome": nome, "passo": passo,
                "giri_attuali": giri_attuali,
                "giri_finali_int": giri_finali_int,
                "tempo_fine": tempo_fine_prevista,
                "fermo": e_fermo,
                "gap_secs": gap_secs,
            })
        # Sort: piu' giri previsti = primo. A parita', chi finisce
        # prima (tempo_fine minore) = primo. I fermi (giri_finali
        # piccolo) finiscono in fondo automaticamente.
        proiezioni.sort(key=lambda x: (-x["giri_finali_int"],
                                         x["tempo_fine"]))
        if not proiezioni:
            return []
        leader = proiezioni[0]
        ...
        for pos, p in enumerate(proiezioni, start=1):
            if p.get("fermo"):
                ...
                delta_giri = (leader["giri_finali_int"]
                              - p["giri_finali_int"])
                if delta_giri >= 1:
                    # Distanziato di N giri interi: stabile,
                    # niente parte frazionaria.
                    gap_str = "-%dg" % delta_giri
                else:
                    # Stesso numero di giri previsti: gap in
                    # secondi = differenza tempo_fine. Stabile
                    # perche' tempo_fine = absoluttime +
                    # giri_aggiuntivi_int * passo (entrambi
                    # discreti, varia solo al cambio di passo
                    # o al cambio di rem_sec_intero).
                    delta_t = p["tempo_fine"] - leader["tempo_fine"]
                    gap_str = "-%.1fs" % max(0.0, delta_t)
            risultato.append({
                "pos": pos,
                "pid": p["pid"],
                "nome": p["nome"],
                "giri_int": p["giri_finali_int"],
                "giri_frac": float(p["giri_finali_int"]),
                "passo": p["passo"],
                "gap_str": gap_str,
    ...
    def _live_aggiorna_prev_passo(self):
        ...
        if in_modo_myrcm:
            ...
            if not pilot_info:
                ...
                # v05.06.51/52: passa LAPS/ABSOLUTTIME/MEDIUMTIME
                # reali dal server MyRCM, cosi' la proiezione usa
                # i valori veri del pilota anche per i giri pre-
                # baseline (mid-session). Senza questi override:
                # - giri_attuali = 1 invece di 5 (proiezione
                #   sbagliata)
                # - absoluttime = 24s invece di 120s (gap enorme,
                #   pilota marcato erroneamente come RIT)
                # - passo basato su 1 solo giro locale (poco
                #   stabile, MEDIUMTIME del server e' meglio)
                laps_map = getattr(self, "_myrcm_prev_laps", None)
                abs_map = getattr(self, "_myrcm_prev_abs", None)
                med_map = getattr(self, "_myrcm_prev_medium", None)
                cls = self._live_classifica_predittiva(
                    pilot_info, rem_sec,
                    currenttime_sec=currenttime_sec,
                    laps_reali_map=laps_map,
                    abs_reali_map=abs_map,
                    passo_reali_map=med_map)
                for r in cls:
                    pos_str = "%d." % r["pos"]
                    nome_short = r["nome"][:7]
                    prev_lines.append(
    ...
    def attiva_myrcm_live(self, recorder, mid_session=False):
        ...
        # MEDIUMTIME (media giro calcolata dal server MyRCM, su
        # TUTTI i giri del pilota inclusi quelli pre-baseline).
        # Usato come passo gara quando i giri locali post-baseline
        # sono pochi (< 3) - piu' stabile della media di 1-2 giri.
        self._myrcm_prev_medium = {}  # pilot_num -> float (secondi)
    ...
    def _myrcm_pilot_num(p):
        """Estrae un identificativo numerico univoco del pilota
        per usarlo come chiave di `_live_pilots`.

        Strategia (v05.06.48):
        1. PILOTNUMBER se > 0 (numero gara assegnato, es. CAT GT)
        2. TRANSPONDER come int (univoco per chip, eventi ENS / pro
           dove i piloti hanno tutti PILOTNUMBER=0)
        3. INDEX (posizione nella lista DATA, fallback finale)

        Bug v05.06.47: in eventi ENS tutti i piloti avevano
        PILOTNUMBER=0, quindi tutti finivano nella stessa chiave
        del dict _live_pilots (= 1 sola colonna visibile per 11
        piloti reali).
        """
        if not isinstance(p, dict):
            return None
        # 1. PILOTNUMBER se valido (> 0)
        try:
            ...
            pass
        # 2. TRANSPONDER come int (univoco per chip; gli ENS lo
        #    usano come identificativo principale visto che il
        #    PILOTNUMBER e' 0 per tutti)
        try:
            t = str(p.get("TRANSPONDER", "")).strip()
            # Estrai solo le cifre (formato MyRCM puo' essere
            # "1234567" o "1234567/0" con slash + sub-id chip)
            digits = "".join(ch for ch in t if ch.isdigit())
            if digits:
                # Ultime 7 cifre per stare in range int piccolo
                return int(digits[-7:])
        except (ValueError, TypeError):
            pass
        # 3. INDEX come fallback (sempre disponibile, posizione
        #    nella lista DATA dell'EVENT). Offset 100000000 per
        #    non collidere con PILOTNUMBER o TRANSPONDER reali.
        try:
    ...
    def _myrcm_aggiorna_cronometro(self, metadata):
        ...
        # v05.06.55: aggiorna _lbl_session_info sotto il cronometro
        # con quello che stiamo effettivamente cronometrando.
        # Formato: "<categoria/sezione>  -  <fase>  -  <manche>"
        # Es: "ENS 1:8 SPORT  -  Controlled practice  -  Group 5 - Heat 1"
        # Cosi' a colpo d'occhio l'utente sa CHE manche sta seguendo.
        try:
            lbl_si = getattr(self, "_lbl_session_info", None)
            if lbl_si is not None and lbl_si.winfo_exists():
                section = (metadata.get("SECTIONNAME")
                           or metadata.get("SECTION") or "").strip()
                group = (metadata.get("GROUP") or "").strip()
                # GROUP MyRCM tipico: "CAT :: FASE :: BODY".
                # Splitto su "::" per dare formato leggibile.
                # Se SECTIONNAME e' gia' nel GROUP, evito doppione.
                parts = [p.strip() for p in group.split("::")
                         if p.strip()]
                if section and parts and section.lower() == parts[0].lower():
                    parts = parts[1:]
                pieces = []
                if section:
                    pieces.append(section)
                pieces.extend(parts)
                testo = "  -  ".join(pieces) if pieces else ""
                lbl_si.config(text=testo, fg=col_tempo)
        except (tk.TclError, Exception):
            pass
    ...
    def _myrcm_on_event(self, metadata, data):
        ...
        # Log diagnostico al PRIMO on_event (per sessione LapTimer):
        # numero piloti ricevuti vs colonne create. Aiuta a capire
        # se MyRCM ne trasmette pochi o se TM ne crea pochi.
        n_data = len(data or [])
        if not getattr(self, "_myrcm_log_first_event_done", False):
            self._myrcm_log_first_event_done = True
            piloti_ids = []
            for p in (data or [])[:8]:
                pn = self._myrcm_pilot_num(p)
                nome = (p.get("PILOT", "") or "").strip()[:18]
                piloti_ids.append("#%s(%s)" % (pn, nome))
            print("[laptimer MyRCM] primo on_event: n_piloti_in_data="
                  "%d (esempi: %s) | colonne attuali=%d"
                  % (n_data, ", ".join(piloti_ids),
                     len(self._live_pilots)))
        ...
        for p in (data or []):
            ...
            # MEDIUMTIME = media giro del server MyRCM (calcolata
            # su TUTTI i giri, anche pre-baseline). Aggiornato ad
            # ogni EVENT, usato come fallback per il passo gara
            # quando i giri locali post-baseline sono pochi.
            med_now = self._parse_myrcm_time(p.get("MEDIUMTIME"))
            if med_now > 0:
                self._myrcm_prev_medium[pn] = med_now
    ...
    def _live_stop_e_salva(self):
        ...
        scouting_dir = ""
        if self.dati_dir:
            # Determina la cartella scouting in modo robusto:
            #   - se dati_dir e' '<...>/dati'         -> dati/scouting
            #   - se dati_dir e' '<...>/dati/id_XXXX' -> il parent + /scouting
            #   - se dati_dir e' '<...>/scouting'     -> usa dati_dir stesso
            base = self.dati_dir.rstrip("/\\")
            base_name = os.path.basename(base).lower()
            if base_name == "scouting":
                scouting_dir = base
            elif base_name == "dati":
                scouting_dir = os.path.join(base, "scouting")
            else:
                # Subdirectory di dati/ (es. dati/id_XXXX): scouting
                # condiviso a livello del parent
                parent = os.path.dirname(base)
                scouting_dir = (os.path.join(parent, "scouting")
                                if parent else os.path.join(base, "scouting"))
            try:
                os.makedirs(scouting_dir, exist_ok=True)
            except Exception:
                scouting_dir = ""
        ...
        if scouting_dir and self._live_pilots:
            ...
            for pnum, d in self._live_pilots.items():
                ...
                # Aggancio al setup: se LapTimer e' stato lanciato dal
                # setup (self.record_id non vuoto), TUTTI i file salvati
                # in questa sessione vengono marcati con setup_record_id.
                # Cosi' Dr. IA / CONFRONTA li trovano in modo certo,
                # indipendentemente dal nome pilota (anche transponder
                # anonimi tipo 'Trasp. 1' vengono agganciati).
                try:
                    if self.record_id:
                        sess["setup_record_id"] = self.record_id
                    # setup_snapshot lo mettiamo solo per il pilota del
                    # setup corrente (e' la sua scheda tecnica)
                    if (self.pilota and nome_out.lower() == self.pilota.lower()
                            and self.setup_snapshot):
                        sess["setup_snapshot"] = dict(self.setup_snapshot)
                except Exception:
    ...
    def _salva_risultati(self):
        ...
        if hasattr(self, '_salva_path') and self._salva_path and os.path.exists(self._salva_path):
            ...
            # Se il save_dir punta a una cartella 'scouting' (caso
            # cronometro libero da menu SCOUTING), usa la struttura
            # ad albero <anno>/<pista>/. Altrimenti (cronometra dal
            # setup -> save_dir = dati/) salva piatto: sara' poi
            # _copia_in_scouting di crono.py a duplicarlo nell'albero.
            is_scouting = (os.path.basename(
                save_dir.rstrip("/\\")).lower() == "scouting")
            if is_scouting:
                # Helper unificato: path + open + dump + invalida cache
                path = _scouting_paths.salva_sessione(
                    risultato, save_dir, nome)
                if path:
                    self._salva_path = path
                return path
            # Caso 'dati/' (cronometro dal setup): salvataggio piatto.
            # La copia nell'albero scouting la fa _copia_in_scouting
            # di crono.py.
