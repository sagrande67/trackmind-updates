...
class MyRcmLiveRecorder(object):
    ...
    def _on_event(self, metadata, data):
        ...
        # v05.06.43: dump diagnostico al primo EVENT per
        # identificare il campo "Divergenza Tabella Oraria"
        # esposto nativamente dal sito MyRCM. Il file viene
        # scritto una sola volta per sessione recorder.
        if not getattr(self, "_dump_metadata_done", False):
            self._dump_metadata_done = True
            try:
                import json as _json
                base = os.path.dirname(
                    os.path.dirname(os.path.abspath(__file__)))
                dump_path = os.path.join(
                    base, "dati", "myrcm_ws_dump.json")
                with open(dump_path, "w", encoding="utf-8") as f:
                    _json.dump({
                        "metadata": metadata or {},
                        "data_first_pilot": (data or [{}])[0],
                        "n_piloti": len(data or []),
                    }, f, ensure_ascii=False, indent=2)
                # Cerca campi candidati per "divergenza"
                meta = metadata or {}
                candidati = [
                    "TIMETABLEDIFFERENCE", "TIMETABLEDIFF",
                    "DIFFERENCETIMETABLE", "DIVERGENCETIMETABLE",
                    "TIMETABLEOFFSET", "SCHEDULEDIFF",
                    "DIFFERENCE", "DIVERGENCE", "OFFSET",
                    "DIFFERENCETT", "TTDIVERGENCE",
                ]
                trovati = {k: meta[k] for k in candidati
                           if k in meta}
                print("[recorder DUMP] campi METADATA disponibili:")
                for k in sorted(meta.keys()):
                    v = meta[k]
                    print("  %s = %r" % (k, v))
                print("[recorder DUMP] candidati 'divergenza' "
                      "trovati: %s" % (trovati or "NESSUNO"))
                print("[recorder DUMP] dump completo: %s"
                      % dump_path)
            except Exception as _e:
                print("[recorder DUMP] errore:", _e)
