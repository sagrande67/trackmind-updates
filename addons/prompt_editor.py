...
class PromptEditor:
    ...
    def __init__(self, master, dpi=1.0, on_close=None,
                 file_path=None, default_text=None, titolo=None):
        """Apre l'editor notepad retro.
        Param:
          master: Tk parent
          dpi: scaling DPI
          on_close: callback alla chiusura
          file_path: path del file da editare (default: ai_prompt.txt).
                     Se non esiste viene creato al salvataggio.
          default_text: testo "DEFAULT" usato dal bottone RIPRISTINA.
                        Se None, fallback a PROMPT_DEFAULT (system prompt).
          titolo: titolo della finestra (default: "PROMPT IA - Editor").
        """
        global _DPI_SCALE
        _DPI_SCALE = dpi
        ...
        self._prompt_path = file_path or _get_prompt_path()
        # Memorizza il testo default custom (per la modalita' checklist)
        ...
        self.win.title(titolo or "PROMPT IA - Editor")
    ...
    def _ripristina_default(self):
        """Ripristina il testo di default nell'editor (non salva su
        disco). Usa `self._default_text` (impostato dal costruttore)
        cosi' funziona sia col system prompt IA sia con checklist
        gara (o qualsiasi altro testo di default custom)."""
        self._txt.delete("1.0", "end")
        self._txt.insert("1.0", self._default_text)
