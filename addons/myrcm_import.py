...
# Helper centralizzato per la struttura dati/scouting/<anno>/<pista>/.
# (vedi core/scouting_paths.py). L'import e' robusto sia in esecuzione
# normale (TrackMind avviato dalla root) sia in test isolati.
try:
    from core import scouting_paths as _scouting_paths
except ImportError:  # esecuzione standalone dalla cartella addons/
...
def _bootstrap_cache_da_scouting(scouting_dir):
    ...
    nuove = 0
    try:
        # Itera ricorsivamente nella nuova struttura <anno>/<pista>/
...
def salva_scouting(sessione, scouting_dir, transponder_suffix="", setup_snapshot=None):
    ...
    Il file viene messo nella sotto-cartella <anno>/<pista>/ dedotta
    dai metadati della sessione (vedi core.scouting_paths)."""
    ...
    return _scouting_paths.salva_sessione(sessione, scouting_dir, filename)
        ...
        for batteria in batterie:
            ...
            for pilota, giri in tempi.items():
                ...
                if sessione:
                    ...
                    rimossi_dedup = False
                    for old_f, old_path in _scouting_paths.elenca_lap_files(
                            scouting_dir):
                        try:
...
def pulisci_scouting_data_vecchia(scouting_dir, data_corrente):
    ...
    eliminati = 0
    # Scansione ricorsiva nella nuova struttura <anno>/<pista>/.
    for f, path in _scouting_paths.elenca_lap_files(scouting_dir):
        try:
