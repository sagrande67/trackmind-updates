...
# Interruttore globale: se False, tutte le chiamate a proteggi_finestra()
# diventano no-op (niente topmost, niente lift periodico). Utile su PC
# desktop dove l'utente non vuole TrackMind sempre sopra le altre finestre.
# Si imposta una volta sola all'avvio (vedi retrodb.main()) leggendo il
# flag "topmost_attivo" dalla configurazione.
_topmost_enabled = True


def set_topmost_enabled(enabled):
    """Abilita o disabilita globalmente la protezione topmost.

    Chiamare PRIMA della prima chiamata a proteggi_finestra(). Quando
    impostato a False, proteggi_finestra() ritorna subito senza fare
    nulla, lasciando la finestra come una qualunque altra (utile su PC).
    """
    global _topmost_enabled
    _topmost_enabled = bool(enabled)


def topmost_enabled():
    """Ritorna lo stato corrente dell'interruttore globale."""
    return _topmost_enabled
...
def proteggi_finestra(root, intervallo_ms=_INTERVALLO_MS_DEFAULT, topmost=True):
    ...
    Se l'interruttore globale e' False (vedi set_topmost_enabled), ritorna
    subito senza alcun effetto.
    """
    # Interruttore globale: utente ha disattivato topmost da conf
