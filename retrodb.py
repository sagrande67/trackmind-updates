...
class RetroDBApp:
    ...
    def _schermata_login(self):
        ...
        # Check aggiornamenti automatico PRIMA del login
        if _HAS_UPDATER and not getattr(self, '_aggiornamento_scaricato', False):
            self._login_blocca(True)
            self._login_status.config(text="Verifica aggiornamenti...", fg=c["testo_dim"])
            self.root.after(500, self._check_update_al_login)
    ...
    def _schermata_menu(self):
        ...
        if is_admin(self.sessione):
            ...
            if _HAS_UPDATER:
                ...
                _bb.append(_mkb(btn_grid, "PREPARA", self._schermata_prepara_aggiornamento))
