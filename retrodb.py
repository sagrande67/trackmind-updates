...
# Gestione centralizzata dei path della cartella scouting (struttura
# ad albero <anno>/<pista>/). Vedi core/scouting_paths.py.
...
# Doctor IA (add-on TrackMind - sintomo guida -> setup proposto IA)
try:
    from doctor_ia import DoctorIA as _DoctorIA
    _HAS_DR_IA = True
except ImportError:
    _HAS_DR_IA = False
    _DoctorIA = None
...
class TableDef:
    ...
    OPERAZIONI_DEFAULT = {
        ...
        "dr_ia": False,     # Bottone DR. IA: il pilota indica un sintomo
                            # di guida e l'IA propone modifiche al setup
                            # (richiede campi marcati ;A nel .def)
    }
...
class RetroDBApp:
    ...
    def __init__(self, root):
        ...
        # Pre-attiva il flag "login attivo" cosi' la primissima esecuzione
        # del monitor (subito sotto) non parte mentre stiamo ancora
        # costruendo la finestra di login: il subprocess nmcli + TCP-check
        # bloccherebbe il thread Tk per ~1s rallentando l'avvio.
        self._login_attivo = True
        self._wifi_monitor()
    ...
    def _schermata_login(self):
        ...
        # Sospende _wifi_monitor durante il login: il check fa subprocess
        # nmcli (timeout 5s) + TCP connect verso 1.1.1.1:53 (timeout 1s)
        # ed essendo bloccante sul thread principale, freezzava il cursore
        # mentre l'utente digita la password ogni 5 secondi.
        # La label Wi-Fi visibile sotto il form viene comunque calcolata
        # qui sotto in modo sincrono (una volta sola), quindi l'utente vede
        # lo stato corrente all'apertura della schermata.
        self._login_attivo = True
    ...
    def _schermata_menu(self):
        self._pulisci(); c = carica_colori()
        # Login completato: riattiva _wifi_monitor (era sospeso per non
        # bloccare il cursore della password con il TCP-check di rete).
        self._login_attivo = False
        ...
        # v05.06.54: rimossa la label "GARA: ..." (sia overlay sul
        # Toplevel che riga nel menu retrodb). L'utente la trovava
        # esteticamente brutta e poco utile (il countdown si vede
        # gia' aprendo Assistente Gara). Il monitor in background
        # resta attivo: countdown + alert beep continuano a girare
        # invisibili, l'utente puo' aprire la UI dal menu quando
        # serve.
    ...
    def _wifi_monitor(self):
        ...
        Se siamo nella schermata di login (_login_attivo=True) il check e'
        sospeso: il subprocess nmcli + TCP-connect bloccano il thread di
        Tk per ~1s e questo freezzava la digitazione della password."""
        if getattr(self, "_login_attivo", False):
            # Sospeso: ricontrolla tra 1.5s in modo che appena l'utente
            # esce dal login il monitor riparta subito.
            self.root.after(1500, self._wifi_monitor)
            return
        try:
            connesso, ssid = self._wifi_stato()
    ...
    def _lancia_crono(self):
        ...
        Crono(parent=self._vista, on_close=self._ritorno_da_crono,
              contesto=contesto,
              on_apri_setup=self._apri_setup_da_crono)
        self._rimuovi_coperta()

    def _apri_setup_da_crono(self, idx):
        """Callback chiamato da Crono quando l'utente preme 'VAI AL
        SETUP' su una riga di sessione: apre la scheda di setup il
        cui indice e' idx (nel db dei setup corrente)."""
        if idx is None or idx < 0:
            return
        self._crono_ritorno_record = int(idx)
        # _ritorno_da_crono ricostruira' la scheda al record indicato
        self._ritorno_da_crono()

    def _lancia_doctor_ia(self):
        """Lancia il modulo Dr. IA: il pilota indica un sintomo di
        guida, l'IA propone modifiche al setup attuale."""
        if not _HAS_DR_IA or _DoctorIA is None:
            self._status("Modulo Dr. IA non disponibile!", "stato_errore")
            return
        if self.indice_corrente is None or self.indice_corrente < 0:
            self._status("Apri prima un setup esistente per "
                          "consultare il Dr. IA.", "stato_avviso")
            return
        rec = self.db.leggi(self.indice_corrente)
        if not rec:
            self._status("Record corrente non valido.", "stato_errore")
            return
        # Verifica che la tabella abbia almeno un campo marcato ;A
        if not any(c.get("analisi_ia") for c in self.table_def.campi):
            self._status("Nessun campo marcato per analisi IA in "
                          "questa tabella (flag ;A nel .def).",
                          "stato_avviso")
            return
        # Costruisci contesto IA dal setup corrente (riusiamo la stessa
        # logica di Crono che gia' raccoglie pista/telaio/motore/gomme/
        # meteo + parametri_ia).
        contesto = self._build_crono_contesto()
        # Memorizza nome e indice per il back/save
        nome_tab = self._nome_tabella
        idx_base = self.indice_corrente

        # Callback: torna al form del setup originale
        def _on_back():
            self._pulisci()
            self.indice_corrente = idx_base
            self._aggiorna_visibili()
            try:
                self._pos_visibile = self._indici_visibili.index(idx_base)
            except (ValueError, AttributeError):
                self._pos_visibile = 0
            self._costruisci_form(nome_tab)
            self._mostra_record()
            self._status("Dr. IA chiuso.", "testo_dim")

        # Callback: applica le modifiche IA copiando il setup base
        # in un nuovo record (l'utente puo' ancora ritoccare prima di
        # premere SALVA del form standard).
        def _on_save(modifiche, spiegazione, sintomo):
            try:
                self._pulisci()
                self._copia_setup(nome_tab, idx_base,
                                  override_valori=modifiche,
                                  nota_ia=("[Dr.IA - %s] %s" %
                                           (sintomo or "?",
                                            spiegazione or ""))[:200])
                self._status("Setup IA pre-compilato. Modifica e premi "
                              "SALVA per confermare.", "stato_ok")
            except Exception as e:
                self._status("Errore creazione nuovo setup: %s" % e,
                              "stato_errore")

        self._pulisci()
        _DoctorIA(parent=self._vista,
                  record=rec,
                  table_def=self.table_def,
                  contesto_extra=contesto,
                  on_back=_on_back,
                  on_save=_on_save)
        self._rimuovi_coperta()
    ...
    def _elimina_file_tempi(self, idx):
        ...
        Scansiona ricorsivamente la cartella ``dati/scouting/`` per
        gestire anche la struttura ad albero <anno>/<pista>/."""
        ...
        if not dati_dir or not os.path.isdir(dati_dir):
            return
        scouting_dir = os.path.join(dati_dir, "scouting")
        ...
        eliminati = 0
        # Scansione ricorsiva della struttura <anno>/<pista>/ + retrocompat
        # per file ancora piatti in dati_dir o in dati/scouting/ piatto.
        candidati = []
        # 1) Sotto-cartelle scouting (nuova struttura)
        for basename, full in _scouting_paths.elenca_lap_files(scouting_dir):
            candidati.append((basename, full))
        # 2) Eventuali file legacy ancora piatti in dati_dir
        if os.path.isdir(dati_dir):
            for f in os.listdir(dati_dir):
                full = os.path.join(dati_dir, f)
                if os.path.isfile(full) and f.endswith(".json"):
                    candidati.append((f, full))

        for basename, full in candidati:
    ...
    def _bootstrap_assistente_gara(self):
        ...
        if not _mon.attivo:
            try:
                ...
                pass
        # v05.06.54: rimossa la creazione del widget overlay
        # _lbl_assist_gara (era posizionato al centro alto del
        # Toplevel e copriva i titoli delle schermate). Il
        # monitor resta attivo invisibile.
    ...
    def _aggiorna_widget_assistente(self):
        """v05.06.54: no-op. La label "GARA: ..." (sia nel menu
        che overlay sul Toplevel) e' stata rimossa per richiesta
        utente. Il monitor in background continua a girare
        invisibile (countdown + alert beep). Metodo conservato
        come no-op per retrocompat con eventuali chiamate
        esterne."""
        return
        # NB: il vecchio body sotto e' irraggiungibile, mantenuto
        # solo a titolo storico per future reintegrazioni.
        if not _HAS_ASSISTENTE:
            return
        try:
            _mon = AssistenteGaraMonitor.get(self.root)
    ...
    def _applica_conf(self):
        ...
        # Applica topmost in tempo reale (richiede riavvio per gli addon
        # gia' aperti, ma per la finestra principale si attiva/disattiva subito)
        try:
            from core.focus_guard import set_topmost_enabled, sblocca_finestra
            topmost_on = int(self.conf.get("topmost_attivo", 1)) == 1
            set_topmost_enabled(topmost_on)
            if not topmost_on:
                # Rimuove subito l'attributo dalla finestra principale
                sblocca_finestra(self.root)
            else:
                # Riattiva immediatamente sulla principale
                try:
                    from core.focus_guard import proteggi_finestra_sicura
                    proteggi_finestra_sicura(self.root)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def _copia_setup(self, nome_tabella, idx,
                     override_valori=None, nota_ia=None):
        """Clona un setup esistente in un nuovo record con data aggiornata.

        Args:
            nome_tabella: nome della tabella corrente
            idx: indice del record sorgente
            override_valori: dict opzionale {NomeCampo: nuovo_valore}
                con valori che SOSTITUISCONO quelli del record sorgente
                (usato dal Dr. IA per pre-compilare il nuovo setup con
                le modifiche proposte dall'IA).
            nota_ia: testo opzionale da appendere al campo "Note" del
                nuovo record (per tracciare la genesi del setup IA).
        """
        if idx is None: return

        rec = self.db.leggi(idx)
        if not rec: return
        # Normalizza override (chiavi case-sensitive come nei campi .def)
        override_valori = override_valori or {}
        ...
        for campo in self.table_def.campi:
            ...
            if n in self.fields:
                ...
                # Override IA: se il Dr. IA ha proposto un nuovo valore
                # per questo campo, usa quello al posto dell'originale.
                if n in override_valori:
                    val = override_valori[n]
                # Append nota IA al campo Note (se esiste e nota_ia ha valore)
                if nota_ia and n.lower() in ("note", "note_ia"):
                    val_str = str(val).strip()
                    val = (val_str + "\n" + nota_ia).strip() if val_str else nota_ia
                if val != "" and val is not None:
                    self.fields[n].set(str(val))
    ...
    def _costruisci_form(self, nome_tabella):
        ...
        # Bottone DR. IA: il pilota indica un sintomo, l'IA propone un
        # nuovo setup. Compare solo se nel .def e' attivo !dr_ia;vero
        # (e l'addon e' caricato).
        if self.table_def.puo("dr_ia") and _HAS_DR_IA:
            tk.Label(bar, text=" ", bg=c["sfondo"], width=1).pack(side="left")
            btn_dr_ia = tk.Button(bar, text="DR. IA ^I",
                      font=self._f_btn_tab, width=_S(8),
                      bg=c["pulsanti_sfondo"], fg=c["stato_avviso"],
                      activebackground=c["pulsanti_sfondo"],
                      activeforeground=c["stato_avviso"],
                      relief="ridge", bd=1, cursor="hand2", pady=0)
            btn_dr_ia.config(command=self._flash_btn(btn_dr_ia,
                                                    self._lancia_doctor_ia))
            btn_dr_ia.pack(side="left", padx=_S(1))
            _bar_btns.append(btn_dr_ia)
            self._btn_map["dr_ia"] = btn_dr_ia
        ...
        # Dr. IA (solo se !dr_ia;vero nel .def + addon caricato)
        if self.table_def.puo("dr_ia") and _HAS_DR_IA:
            _bind_ctrl("i", lambda e: self._flash_key("dr_ia",
                                                     self._lancia_doctor_ia))
        # Escape -> torna indietro
        if self.table_def.is_composite:
            self.root.bind("<Escape>", lambda e: self._schermata_selezione(nome_tabella))
...
def main():
    ...
    # L'utente puo' disattivarlo da conf (topmost_attivo=0) su PC desktop dove
    # non serve la protezione popup ed e' fastidioso avere TrackMind sempre sopra.
    try:
        from core.focus_guard import set_topmost_enabled
        set_topmost_enabled(int(app.conf.get("topmost_attivo", 1)) == 1)
    except Exception:
        pass
